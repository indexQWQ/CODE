...
int main() 
{
    ...
    point arr[10000]; // 存储每个数字的坐标
    int pointCount=0; // 记录点的数量
    int maxDistance=0; // 记录最大平方距离
    // 输入矩阵并记录每个数字的坐标
    int i=0;
    for (i=0;i<n;i++) 
    {
    ...
    for (i=0;i<pointCount;i++) 
    {
        ...
        {
            if (matrix[arr[i].x][arr[i].y]==matrix[arr[j].x][arr[j].y]) 
            {
                int distance=calculate(arr[i],arr[j]);
